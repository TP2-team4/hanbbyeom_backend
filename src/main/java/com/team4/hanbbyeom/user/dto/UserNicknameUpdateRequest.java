...
public record UserNicknameUpdateRequest(
        ...
        // 회원가입(SignUpRequest)과 같은 규칙을 적용한다 — 가입 때는 되던 닉네임이 변경 때는 막히거나 그 반대가
        // 되지 않도록 하기 위함. 서비스 정책상 좁게 제한하고 DB(50자)는 향후 정책 변경 대비 여유값이다.
        // 중복은 회원가입과 동일하게 허용한다(DB에 유니크 제약이 없다).
