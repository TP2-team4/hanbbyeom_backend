...
// status는 activity_match.status(PROPOSED/CONFIRMED/REJECTED/CANCELLED/EXPIRED/ENDED)를 그대로 노출하지
...
//   CANCELLED: 신청자 본인이 직접 취소한 경우(REJECTED 또는 확정 후 취소된 CANCELLED, closedByUserId=신청자 본인).
//              호스트가 확정 후 취소했거나 탈퇴로 시스템이 취소한 건은 "내 신청이 성사되지 않음"이라 REJECTED로 보인다
